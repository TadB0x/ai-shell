...
export async function generateCommand(query: string, apiKey: string): Promise<CommandResult> {
  ...
  try {
    ...
    if (err instanceof Anthropic.AuthenticationError) throw new Error('Invalid Anthropic API key. Run `ai config` to update it.')
    if (err instanceof Anthropic.RateLimitError) throw new Error('Anthropic rate limit reached. Try again in a moment.')
